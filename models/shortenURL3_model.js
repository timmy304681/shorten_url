...
const getLongUrl1 = async (shortUrl) => {
  const [[result]] = await dbRead1.execute('SELECT long_url FROM `url_info` WHERE short_url = ?', [
    shortUrl,
  ]);
  return result;
};
...
const getLongUrl2 = async (shortUrl) => {
  const [[result]] = await dbRead2.execute('SELECT long_url FROM `url_info` WHERE short_url = ?', [
    shortUrl,
  ]);
  return result;
};
...
const getLongUrl3 = async (shortUrl) => {
  const [[result]] = await dbRead3.execute('SELECT long_url FROM `url_info` WHERE short_url = ?', [
    shortUrl,
  ]);
  return result;
};
