...
async function getShortURL(longURL) {
  ...
  const shortURL = process.env.EC2_NUM + 'A'.repeat(6 - encodedId.length) + encodedId;

  // insert shortURL
  await dbWrite.execute('UPDATE url_info SET short_url = ? WHERE id = ?', [shortURL, id]);
  return shortURL;
}

async function getLongURLFromDb1(id) {
  const [[result]] = await dbRead1.execute('SELECT long_url FROM url_info WHERE id = ?', [id]);
  return result;
}

async function getLongURLFromDb2(id) {
  const [[result]] = await dbRead2.execute('SELECT long_url FROM url_info WHERE id = ?', [id]);
  return result;
}

async function getLongURLFromDb3(id) {
  const [[result]] = await dbRead3.execute('SELECT long_url FROM url_info WHERE id = ?', [id]);
  return result;
}
